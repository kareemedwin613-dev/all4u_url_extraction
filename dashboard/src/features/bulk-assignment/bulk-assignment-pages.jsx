...
function capacityState(proposals, workloads) {
  const map = new Map(workloads.map((workload) => [workload.userId, { ...workload, count: 0 }]));
  for (const proposal of proposals) {
    const workload = map.get(proposal.proposedAssigneeId);
    if (workload) workload.count += 1;
  }
  const summaries = [...map.values()]
    .filter((workload) => workload.count)
    .map((workload) => ({
      ...workload,
      projected: workload.activeApplicationCount + workload.count,
      valid: workload.activeApplicationCount + workload.count <= workload.maxActiveApplications,
    }));
