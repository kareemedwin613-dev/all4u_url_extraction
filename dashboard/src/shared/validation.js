...
export function validateConfig(env={}){const url=String(env.VITE_SUPABASE_URL||"").trim(),key=String(env.VITE_SUPABASE_PUBLISHABLE_KEY||"").trim();let parsed;try{parsed=new URL(url);}catch{}const errors=[],privilegedMarker=["service","role"].join("_");if(!parsed||parsed.protocol!=="https:"||!parsed.hostname.endsWith(".supabase.co")||parsed.pathname!=="/")errors.push("Set VITE_SUPABASE_URL to a standard HTTPS Supabase project URL.");if(key.length<20||key.toLowerCase().includes(privilegedMarker))errors.push("Set VITE_SUPABASE_PUBLISHABLE_KEY to a publishable or anon browser key.");return {valid:!errors.length,errors,config:{url:parsed?`${parsed.protocol}//${parsed.hostname}`:"",key}};}
