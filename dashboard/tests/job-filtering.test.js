...
test("JD list sends bearer-authenticated search, filters, sorting, and pagination to the backend",async()=>{
  ...
  const categoryId="b4d63a80-e306-4a2f-afca-29cd4b3951e0",capturedByUserId="7d27656a-1f62-4708-9cd4-2eb659715116";
  try{
    const result=await listJobs(client,"https://api.example.com",{search:"data",categoryId,seniority:"SENIOR",status:"ACTIVE",capturedByUserId,sort:"company_asc",page:1,pageSize:25});
    const url=new URL(request.url);
    assert.equal(url.pathname,"/api/v1/job-descriptions");
    assert.deepEqual(Object.fromEntries(url.searchParams),{search:"data",categoryId,seniority:"SENIOR",status:"ACTIVE",capturedByUserId,sort:"company_asc",page:"1",pageSize:"25"});
