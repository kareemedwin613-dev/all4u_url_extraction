...
import{archiveResumeAnswer,createResumeAnswer,listResumeAnswers,saveResumeAnswers,updateResumeAnswer}from"../src/features/resume-answers/resume-answer-service.js";
...
test("Resume Answer Library uses authenticated backend CRUD routes only",async()=>{
 ...
 try{await listResumeAnswers(client,"https://api.example.com",resumeId);await saveResumeAnswers(client,"https://api.example.com",resumeId,[{answerKey:"authorized_to_work"}]);await createResumeAnswer(client,"https://api.example.com",resumeId,{answerKey:"authorized_to_work"});await updateResumeAnswer(client,"https://api.example.com",resumeId,answerId,{active:false});await archiveResumeAnswer(client,"https://api.example.com",resumeId,answerId);}finally{globalThis.fetch=original;}
 assert.deepEqual(calls.map(({url,options})=>[new URL(url).pathname,options.method]),[[`/api/v1/resumes/${resumeId}/application-answers`,`GET`],[`/api/v1/resumes/${resumeId}/application-answers`,`PATCH`],[`/api/v1/resumes/${resumeId}/application-answers`,`POST`],[`/api/v1/resumes/${resumeId}/application-answers/${answerId}`,`PATCH`],[`/api/v1/resumes/${resumeId}/application-answers/${answerId}`,`DELETE`]]);
