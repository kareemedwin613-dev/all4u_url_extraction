...
test("each fixed role maps to the expected capabilities",()=>{
  assert.deepEqual([...capabilitiesForRoles(["APPLIER"])].sort(),[CAPABILITIES.BUSINESS_DATA_READ,CAPABILITIES.PROFILE_VIEW_SELF].sort());
  assert.deepEqual([...capabilitiesForRoles(["APPLYING_MANAGER"])].sort(),[CAPABILITIES.BUSINESS_DATA_READ,CAPABILITIES.EXTENSION_BUSINESS_WRITE,CAPABILITIES.PROFILE_VIEW_SELF].sort());
...
test("multiple roles form a deduplicated union and unknown roles grant nothing",()=>{
  ...
  assert.deepEqual([...capabilitiesForRoles(["APPLIER","DEVELOPER"])].sort(),[CAPABILITIES.BUSINESS_DATA_READ,CAPABILITIES.PROFILE_VIEW_SELF].sort());
});
