# Resume JD Capture and Operations Platform v1.9
...
The fixed `JD_FINDER` role provides least-privilege capture access. An active JD Finder can browse the shared Job Description catalog, load controlled category/industry lookups, save JDs attributed to their own authenticated user, and correct their own unapproved captures, but cannot review JDs, or access Applications, Resumes, tailoring, or administration.
...
All new workload and assignment-batch tables use RLS. Authenticated table access is role-scoped and direct writes are denied; preview and assignment use `SECURITY DEFINER` RPCs that verify the active caller. Bulk assignment accepts reviewed Application/Applier IDs, revalidates live eligibility and capacity, and derives the actor from `auth.uid()`. Private Resume Storage and short-lived signed URLs are preserved. v1.6 uses a request-scoped user client, a manager-only reservation token, exact job-specific Storage paths, and protected finalization. Assigned Appliers can read only the tailored object currently attached to their Application.
...
AI workload optimization, scheduled assignment, bulk reassignment, teams, organizations, feature-permission tables, AI matching/scoring, job-site submission, OpenAI API, and every other AI API are intentionally excluded. The local worker uses the locally authenticated Codex CLI only for an operator-triggered preview; the mandatory human decision remains separate. v1.9 renders approved content as DOCX or PDF with fixed code-owned templates.
