...
test("structured Resume save body strips profile-only fields like source",()=>{
  ...
  assert.equal(body.educationLegacyText,"State U — BS CS");
...
test("v3.84 persists editable education legacy text",async()=>{
  const sql=await readFile(new URL("supabase/migrations/202609131600_v3_84_edit_education_legacy_text.sql",root),"utf8");
  const source=await readFile(new URL("dashboard/src/features/candidates/structured-resume-editor.jsx",root),"utf8");
  assert.match(sql,/update_resume_structured_content_v384/);
  assert.match(sql,/education_legacy_text/);
  assert.match(source,/educationLegacyText/);
  assert.match(source,/Clear legacy text/);
  assert.match(source,/Legacy education text/);
});
