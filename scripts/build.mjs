...
const options={entryPoints:{"background/service-worker":resolve(root,"background/service-worker.js"),"content/dashboard-bridge":resolve(root,"content/dashboard-bridge.js"),"content/resume-upload":resolve(root,"content/resume-upload.js"),"content/personal-autofill":resolve(root,"content/personal-autofill.js"),"sidepanel/index":resolve(root,"sidepanel/main.jsx")},bundle:true,outdir:dist,format:"esm",platform:"browser",target:"chrome114",jsx:"automatic",sourcemap:false,minify:true,legalComments:"none"};
...
const manifest=JSON.parse(await readFile(resolve(dist,"manifest.json"),"utf8"));if(manifest.version!=="1.0.1")throw new Error("Built manifest must be version 1.0.1");
