...
const options={entryPoints:{"background/service-worker":resolve(root,"background/service-worker.js"),"content/dashboard-bridge":resolve(root,"content/dashboard-bridge.js"),"sidepanel/index":resolve(root,"sidepanel/main.jsx")},bundle:true,outdir:dist,format:"esm",platform:"browser",target:"chrome114",jsx:"automatic",sourcemap:false,minify:true,legalComments:"none"};
...
const manifest=JSON.parse(await readFile(resolve(dist,"manifest.json"),"utf8"));if(manifest.version!=="0.8.5")throw new Error("Built manifest must be version 0.8.5");
