-- v3.109: Align JD pages with the dashboard's 1000-row maximum, preserve
-- all sort orders, and clamp vanished pages within the same read snapshot.
...
declare
  ...
  v_limit integer := greatest(1, least(coalesce(p_limit, 25), 1000));
  ...
  v_source_url text := left(nullif(btrim(coalesce(p_source_url, '')), ''), 500);
  v_company text := left(nullif(btrim(coalesce(p_company, '')), ''), 100);
  v_job_title text := left(nullif(btrim(coalesce(p_job_title, '')), ''), 100);
  v_sort text := lower(nullif(btrim(coalesce(p_sort, 'created_desc')), ''));
  v_shared_reader boolean := public.has_any_role(array['APPLYING_MANAGER', 'ADMIN', 'JD_FINDER'], v_actor);
  v_applier boolean := public.has_role('APPLIER', v_actor);
  v_total bigint := 0;
  v_items jsonb := '[]'::jsonb;
begin
  if v_actor is null or not public.is_active_user(v_actor) or not (v_shared_reader or v_applier) then
  ...
  with visible as (
    ...
    where (
      ...
      and (
        v_source_url is null
        or strpos(lower(coalesce(j.source_url, '')), lower(v_source_url)) > 0
        or strpos(lower(coalesce(j.normalized_source_url, '')), lower(v_source_url)) > 0
      )
      and (
        v_company is null
        or strpos(lower(coalesce(j.company, '')), lower(v_company)) > 0
      )
      and (
        v_job_title is null
        or strpos(lower(coalesce(j.job_title, '')), lower(v_job_title)) > 0
      )
  ),
  counted as (
  ...
  page as (
    ...
    order by
      ...
      v.id desc
    limit v_limit
    -- Keep the returned rows aligned with the API's clamped page metadata.
    -- Counting and selecting from one snapshot also handles concurrent deletes.
    offset (
      select case when total = 0 then 0
        else least(v_offset::bigint, ((total - 1) / v_limit) * v_limit)
      end from counted
    )
  ),
  shaped as (
      ...
      (
        select jsonb_agg(
          ...
          order by
            ...
            case when v_sort = 'category_asc' then s.category_id::text end asc nulls last,
            case when v_sort = 'category_desc' then s.category_id::text end desc nulls last,
            case when v_sort = 'subcategory_asc' then s.subcategory_id::text end asc nulls last,
            case when v_sort = 'subcategory_desc' then s.subcategory_id::text end desc nulls last,
            case when v_sort = 'seniority_asc' then s.seniority end asc nulls last,
            case when v_sort = 'seniority_desc' then s.seniority end desc nulls last,
            case when v_sort = 'source_asc' then lower(coalesce(s.source_url, '')) end asc nulls last,
            case when v_sort = 'source_desc' then lower(coalesce(s.source_url, '')) end desc nulls last,
            case when v_sort = 'capturer_asc' then s.user_id::text end asc nulls last,
            case when v_sort = 'capturer_desc' then s.user_id::text end desc nulls last,
            case when v_sort = 'status_asc' then s.status end asc nulls last,
            case when v_sort = 'status_desc' then s.status end desc nulls last,
            case when v_sort = 'review_asc' then s.review_status end asc nulls last,
            case when v_sort = 'review_desc' then s.review_status end desc nulls last,
...
revoke all on function public.list_job_descriptions_v396(
  text, uuid, text, text, text, uuid, timestamptz, timestamptz, text, integer, integer, text, text, text
) from public, anon;
...
comment on function public.list_job_descriptions_v396(
  ...
  'Caller-scoped Job Description list with optional company, job title, and source URL filters.';
