-- Applier Productivity + Profile Status: Interviews split by tailored vs non-tailored resume.
...
begin
  ...
  from (
    ...
    select p.id, p.applier_name, p.email, p.profile_status,
      ...
      count(pa.id) filter (where pa.status = 'INTERVIEW_SCHEDULED')::integer interviews_count,
      count(pa.id) filter (
        where pa.status = 'INTERVIEW_SCHEDULED' and pa.resume_type = 'TAILORED'
      )::integer interviews_tailored_count,
      count(pa.id) filter (
        where pa.status = 'INTERVIEW_SCHEDULED' and pa.resume_type = 'ORIGINAL'
...
comment on function public.get_business_overview_v31(timestamptz, timestamptz) is
  'Business overview with Applier productivity, including Interviews split by tailored vs non-tailored resume.';
    ...
    select
      ...
      count(a.id) filter (
        where a.created_at >= p_from
          and a.created_at < p_to
          and a.status = 'INTERVIEW_SCHEDULED'
          and coalesce(app_resume.resume_type, 'ORIGINAL') = 'TAILORED'
      )::integer as interview_tailored_count,
      count(a.id) filter (
        where a.created_at >= p_from
          and a.created_at < p_to
          and a.status = 'INTERVIEW_SCHEDULED'
          and coalesce(app_resume.resume_type, 'ORIGINAL') = 'ORIGINAL'
...
comment on function public.get_applier_resume_profile_workload_v31(timestamptz, timestamptz) is
  'Overview workload by Resume profile; Applied and Interviews each split by tailored vs non-tailored resume.';
