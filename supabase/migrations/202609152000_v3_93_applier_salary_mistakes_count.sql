-- Applier Productivity: count screenshot-review mistakes per Applier for salary.
...
begin
  ...
  from (
    ...
    group by p.id, p.applier_name, p.email, p.profile_status, act.active_days, act.last_activity_at, mk.mistakes_count
  ) x;
...
comment on function public.get_business_overview_v31(timestamptz, timestamptz) is
  'Business overview Applier productivity with screenshot-review mistakes_count for salary (applied_at submissions + interviews − mistakes).';
