-- Tailored / non-tailored Application counts must split Applied only:
-- tailored + non_tailored = applied_count.
...
comment on function public.get_application_counts_v29(timestamptz, timestamptz) is
  'RLS-compatible Application summary. Tailored/non-tailored split Applied status only (sum equals applied_count).';
...
comment on function public.get_business_overview_v31(timestamptz, timestamptz) is
  'Business overview with Applier productivity. Tailored/non-tailored split Applied only (sum equals applied_count).';
