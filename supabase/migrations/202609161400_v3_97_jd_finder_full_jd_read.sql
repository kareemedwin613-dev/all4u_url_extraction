-- v3.97: JD Finders may read the full Job Description catalog (same as managers
-- for SELECT). Capture/insert and own-correction write rules stay unchanged.
...
using (
  (select public.has_any_role(array['APPLYING_MANAGER','ADMIN','JD_FINDER']))
  or (
...
comment on policy "role scoped read jobs" on public.job_descriptions is
  'Managers, Admins, and JD Finders read all JDs; Appliers read only JDs for their assigned Applications.';
...
comment on policy "role scoped read job description subcategories" on public.job_description_subcategories is
  'Managers, Admins, and JD Finders read all subcategory rows; Appliers read rows for assigned Application JDs.';
...
comment on function public.list_job_description_capturers() is
  'Managers, Admins, and JD Finders see all capturers; Appliers see capturers for assigned Application JDs only.';
