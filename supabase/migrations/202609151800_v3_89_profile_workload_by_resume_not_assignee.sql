-- Profile Status: count Applications by Resume profile (parent + tailored children),
-- not by the Application's current assigned Applier.
...
begin
  ...
  from (
    ...
    left join public.resumes app_resume
      on coalesce(app_resume.parent_resume_id, app_resume.id)
        = coalesce(r.parent_resume_id, r.id)
...
comment on function public.get_applier_resume_profile_workload_v31(timestamptz, timestamptz) is
  'Overview workload by Resume profile (parent + tailored children); counts all Applications on the profile, not filtered by assigned Applier.';
