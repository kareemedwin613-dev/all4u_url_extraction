-- v3.107: completed AI reviews approve automatically, including corrections.
-- Historical statuses remain stored for audit; no legacy failures are silently approved.
alter table public.jd_review_items drop constraint jd_review_items_status_check;
alter table public.jd_review_items add constraint jd_review_items_status_check
  check(status in ('PENDING','RUNNING','AI_REVIEWED','FAILED','APPROVED','BLOCKED','NEEDS_ATTENTION','SKIPPED','CANCELLED'));
alter table public.jd_review_batches alter column prompt_version set default 'jd-classify-v4';
...
begin
  ...
  if p_operation='create' then
    ...
    return coalesce((select jsonb_agg(to_jsonb(x)) from (
      select batch_row.*,count(i.id) as total,count(i.id) filter(where i.status not in('PENDING','RUNNING')) as finished,
        count(i.id) filter(where i.status in ('AI_REVIEWED','APPROVED')) as ai_reviewed,
  ...
  if p_operation in ('ticket','retry') and b.prompt_version<>'jd-classify-v4' then
  ...
  if p_operation='detail' then
    ...
    update public.jd_review_items set status='PENDING',attempt_count=0,lease_token=null,lease_expires_at=null,finished_at=null
      where jd_review_items.batch_id=b.id and status='FAILED';
...
declare t public.jd_review_tickets; b public.jd_review_batches; i public.jd_review_items;
  ...
  subs uuid[]; v_result jsonb; key text; old_value jsonb; v_corrected boolean := false;
begin
  ...
  if b.prompt_version<>'jd-classify-v4' then
    raise exception 'JD_REVIEW_LEGACY_BATCH: This older review batch is read-only. Create a saved-JD classification batch.';
  end if;
  if p_operation='next' then
    update public.jd_review_items set status='FAILED',comment='Worker lease expired after three attempts; retry after checking worker logs.',finished_at=now()
    ...
    loop
      ...
      if not found then
        return jsonb_build_object('done',not exists(select 1 from public.jd_review_items where batch_id=b.id and status in('PENDING','RUNNING')),
          'failedCount',(select count(*) from public.jd_review_items where batch_id=b.id and status='FAILED'));
      end if;
    ...
    if public.jd_review_snapshot(j.id) is distinct from i.before_value then
      update public.jd_review_items set status='FAILED',comment='JD changed during review. No fields or review decision were overwritten.',finished_at=now() where id=i.id;
      return jsonb_build_object('status','FAILED');
    end if;
    v_result := p_body->'result'; outcome := v_result->>'outcome'; changes := coalesce(v_result->'changes','{}');
    if outcome is null or outcome not in('AI_REVIEWED','FAILED') or jsonb_typeof(changes)<>'object'
    ...
    if outcome='FAILED' then
      if changes<>'{}'::jsonb or v_result->'applyChanges' is distinct from 'false'::jsonb then
        raise exception 'JD_REVIEW_RESULT_INVALID: Failed reviews cannot write changes.';
      end if;
      -- Technical failures never approve or send the JD back to a human correction queue.
      update public.jd_review_items set status='FAILED',result=v_result,comment=v_result->>'comment',
        after_value=public.jd_review_snapshot(j.id),finished_at=now() where id=i.id;
      return jsonb_build_object('status','FAILED');
    end if;
    if nullif(btrim(j.description_text),'') is null then
      raise exception 'JD_REVIEW_RESULT_INVALID: Saved description is required for AI review.';
    end if;
    for key in select jsonb_object_keys(changes) loop
      if key not in('company','job_title','category_id','subcategory_ids','industry_domain_category_id','seniority','location_text','work_arrangement',
      ...
      old_value := i.before_value->key;
      if old_value=changes->key or (jsonb_typeof(old_value)='array' and jsonb_typeof(changes->key)='array'
        and old_value @> (changes->key) and old_value <@ (changes->key)) then
        changes := changes-key;
      end if;
    end loop;
    if changes<>'{}'::jsonb and v_result->'applyChanges' is distinct from 'true'::jsonb then
      raise exception 'JD_REVIEW_RESULT_INVALID: Explicit supported changes required.';
    end if;
    if outcome='AI_REVIEWED' then
      ...
      -- Keep correction metadata for audit, without requiring another human approval.
      for key in select jsonb_object_keys(changes) loop
        old_value := i.before_value->key;
      ...
      update public.job_descriptions set company=n.company,job_title=n.job_title,category_id=n.category_id,subcategory_id=null,
        ...
        where id=j.id;
      perform public.replace_job_description_subcategories(j.id,n.category_id,subs,false);
    end if;
    v_result := v_result || jsonb_build_object('changes',changes,'corrected',v_corrected,'outcome',outcome);
    note := left(format('AI reviewed [%s, %s; item %s]: Live posting availability not checked. %s%s',
      b.prompt_version,b.model,i.id,case when v_corrected then 'Corrected existing values. ' else '' end,v_result->>'comment'),1000);
