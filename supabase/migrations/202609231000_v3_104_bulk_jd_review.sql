-- Bulk URL review. Existing applications are never changed by this workflow.
create table public.jd_review_batches (
  id uuid primary key default gen_random_uuid(),
  created_by uuid not null references public.profiles(id),
  created_at timestamptz not null default now(),
  cancelled boolean not null default false,
  model text not null default 'gpt-5.6-sol',
  prompt_version text not null default 'jd-review-v1'
);
create table public.jd_review_items (
  id uuid primary key default gen_random_uuid(),
  batch_id uuid not null references public.jd_review_batches(id) on delete cascade,
  job_description_id uuid references public.job_descriptions(id) on delete set null,
  status text not null default 'PENDING' check(status in ('PENDING','RUNNING','APPROVED','BLOCKED','NEEDS_ATTENTION','SKIPPED','CANCELLED')),
  ...
  result jsonb,
  comment text,
  unique(batch_id,job_description_id)
);
create index jd_review_items_queue on public.jd_review_items(batch_id,status,lease_expires_at);
create table public.jd_review_tickets (
  token_hash text primary key,
  batch_id uuid not null references public.jd_review_batches(id) on delete cascade,
  created_by uuid not null references public.profiles(id),
  expires_at timestamptz not null default now()+interval '24 hours'
);
alter table public.jd_review_batches enable row level security;
alter table public.jd_review_items enable row level security;
alter table public.jd_review_tickets enable row level security;
-- Access only through the role-checked functions below, including history reads.
revoke all on public.jd_review_batches, public.jd_review_items, public.jd_review_tickets from anon,authenticated;

create function public.jd_review_snapshot(p_id uuid) returns jsonb
language sql stable security definer set search_path=public,pg_temp as $$
  select to_jsonb(j) || jsonb_build_object('subcategory_ids', public.job_description_subcategory_ids(j.id))
  from public.job_descriptions j where id=p_id
$$;
revoke all on function public.jd_review_snapshot(uuid) from public,anon,authenticated;

create function public.jd_review_manage(p_operation text,p_body jsonb default '{}') returns jsonb
...
create function public.jd_review_runner(p_operation text,p_ticket text,p_body jsonb default '{}') returns jsonb
language plpgsql security definer set search_path=public,extensions,pg_temp as $$
declare t public.jd_review_tickets; b public.jd_review_batches; i public.jd_review_items;
  ...
  subs uuid[]; v_result jsonb; key text;
begin
  ...
  if p_operation='next' then
    ...
    if outcome is null or outcome not in('APPROVED','BLOCKED','NEEDS_ATTENTION') or jsonb_typeof(changes)<>'object'
    ...
    if outcome='BLOCKED' and coalesce(v_result->>'blockReason','') not in('EXPIRED','HYBRID_OR_REMOTE') then
      raise exception 'JD_REVIEW_RESULT_INVALID: A confirmed block reason is required.';
    end if;
    ...
    note := left(format('AI bulk review [%s, %s; item %s]: %s',b.prompt_version,b.model,i.id,v_result->>'comment'),1000);
    if outcome='APPROVED' then
      ...
      if not exists(select 1 from public.categories where id=n.category_id and active and parent_id is null)
        or (n.industry_domain_category_id is not null and not exists(select 1 from public.industry_domain_categories where id=n.industry_domain_category_id and active)) then
    ...
    update public.job_descriptions set
      review_status=case outcome when 'APPROVED' then 'APPROVED' when 'BLOCKED' then 'DECLINED' else 'NEEDS_CORRECTION' end,
      review_decline_reason=case when outcome='BLOCKED' then case when v_result->>'blockReason'='EXPIRED' then 'EXPIRED' else 'NOT_ELIGIBLE' end end,
      review_comment=note,
      application_blocked_at=case when outcome='BLOCKED' then now() else application_blocked_at end,
      application_blocked_by=case when outcome='BLOCKED' then t.created_by else application_blocked_by end,
      application_blocked_notes=case when outcome='BLOCKED' then note else application_blocked_notes end
      where id=j.id;
