-- v3.1: allow a JD Finder to correct only their own unapproved captures.
-- The RPC deliberately leaves manager review fields untouched, so the review
-- audit trail and final decisions cannot be rewritten by the capturer.
...
revoke all on function public.update_my_job_description_v31(uuid,text,text,uuid,uuid,text,text,text,text,text,text,text,text[],text[],boolean,text,numeric,numeric,text,text,text) from public,anon;
grant execute on function public.update_my_job_description_v31(uuid,text,text,uuid,uuid,text,text,text,text,text,text,text,text[],text[],boolean,text,numeric,numeric,text,text,text) to authenticated;

comment on function public.update_my_job_description_v31(uuid,text,text,uuid,uuid,text,text,text,text,text,text,text,text[],text[],boolean,text,numeric,numeric,text,text,text)
is 'Caller-scoped correction of a JD Finder capture while NEEDS_REVIEW or NEEDS_CORRECTION; approved/declined captures stay immutable.';
