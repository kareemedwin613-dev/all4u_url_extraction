-- Profile Workload counts Applications by Resume profile, not only the current Applier assignee.
-- After profile transfer, historical work on that Resume still appears under the profile.
...
comment on function public.get_applier_resume_profile_workload_v31(timestamptz, timestamptz) is
  'Overview workload by Resume profile across all Applications on that profile; own mappings for Appliers, all for Admins.';
