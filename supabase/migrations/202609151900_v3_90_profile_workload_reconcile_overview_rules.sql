-- Profile Status: one row per Resume profile root; reuse Overview period + Applied rules
-- so column totals reconcile with Applier Productivity (same Applications universe).
  ...
  from (
    with period_apps as (
      ...
      where a.assigned_to is not null
        and coalesce(r.parent_resume_id, r.id) is not null
        and (
...
comment on function public.get_applier_resume_profile_workload_v31(timestamptz, timestamptz) is
  'Overview workload by Resume profile root. Uses the same period/Applied rules as Applier Productivity so Admin totals reconcile; not filtered by current assignee.';
