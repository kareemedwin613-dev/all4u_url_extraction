...
- coverLetter: the body of a cover letter for this job: 3 to 4 paragraphs, at most 400 words, paragraphs separated by one blank line, plain text. The system adds the greeting, sign-off, name, and contact details; do not write them. Ground every claim in sourceResume, including sourceResume.coverLetter (the candidate's own base letter, when present): keep its voice and supported facts, but add no experience, metric, or claim that neither states. Tailor the opening and emphasis to jobDescription.jobTitle and jobDescription.company, naming the company naturally. Never use placeholders or brackets.
