-- Profile-only bulk assignment: match Applications to Appliers via applier_resume_profiles.
-- Legacy MANUAL/EVEN/CAPACITY_AWARE values remain valid on historical assignment batches.

alter table public.application_assignment_batches
  drop constraint if exists application_assignment_batches_strategy_check;

alter table public.application_assignment_batches
  add constraint application_assignment_batches_strategy_check
  check (strategy in ('MANUAL', 'EVEN', 'CAPACITY_AWARE', 'PROFILE'));
...
begin
  ...
  loop
    ...
    elsif not v_target.is_available then
      insert into v08_preview_exclusions values (
        v_app.application_id, 'APPLIER_UNAVAILABLE', 'Applier is unavailable for assignment.'
      ) on conflict do nothing;
