-- v3.106: evidence-supported corrections require finder review.
-- Preserve older batch history; new v3 batches still use saved JD text only.
alter table public.jd_review_batches alter column prompt_version set default 'jd-classify-v3';
...
begin
  ...
  if p_operation in ('ticket','retry') and b.prompt_version<>'jd-classify-v3' then
  ...
  if p_operation='detail' then
    ...
    update public.jd_review_items set status='PENDING',attempt_count=0,lease_token=null,lease_expires_at=null,finished_at=null
      where jd_review_items.batch_id=b.id and status='NEEDS_ATTENTION'
        and coalesce(result->'corrected','false'::jsonb)<>'true'::jsonb;
...
declare t public.jd_review_tickets; b public.jd_review_batches; i public.jd_review_items;
  ...
  subs uuid[]; v_result jsonb; key text; old_value jsonb; quote text; v_corrected boolean := false;
begin
  ...
  if b.prompt_version<>'jd-classify-v3' then
  ...
  if p_operation='next' then
    ...
    if outcome='APPROVED' or v_result->'applyChanges'='true'::jsonb then
      ...
      -- Quotes are checked against saved text, not the worker's assertion alone.
      for key in select jsonb_object_keys(changes) loop
        quote := lower(btrim(regexp_replace(coalesce(v_result->'evidence'->>key,''),'\s+',' ','g')));
        if length(quote)<2 or strpos(lower(btrim(regexp_replace(coalesce(j.description_text,''),'\s+',' ','g'))),quote)=0 then
          raise exception 'JD_REVIEW_RESULT_INVALID: Saved-description evidence required for every change.';
        end if;
        old_value := i.before_value->key;
      ...
      if v_corrected then outcome := 'NEEDS_ATTENTION'; end if;
      ...
      -- An uncertain result without supported edits preserves captured facts.
      changes := '{}';
    end if;
    v_result := v_result || jsonb_build_object('changes',changes,'corrected',v_corrected,'outcome',outcome);
    note := left(format('AI saved-JD classification [%s, %s; item %s]: Live posting availability not checked. %s%s',
      b.prompt_version,b.model,i.id,case when v_corrected then 'Corrected existing values; Needs Correction. ' else '' end,v_result->>'comment'),1000);
