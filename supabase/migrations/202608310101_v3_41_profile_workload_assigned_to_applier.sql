-- Profile workload: count Applications assigned to the mapped Applier on each Resume profile.
...
comment on function public.get_applier_resume_profile_workload_v31(timestamptz, timestamptz) is
  'Overview workload by Resume profile for Applications assigned to the mapped Applier; own mappings for Appliers, all for Admins.';

-- Keep Admin scorecard profile workload aligned with the same assignee scope.
