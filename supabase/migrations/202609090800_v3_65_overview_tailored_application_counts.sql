-- Overview: expose tailored vs non-tailored Application counts for Activity Overview
-- and Applier Productivity (based on applications.resume_id -> resumes.resume_type).
...
begin
  ...
  select jsonb_build_object(
    ...
    'tailored', count(*) filter (where resume_type = 'TAILORED'),
    'non_tailored', count(*) filter (where resume_type = 'ORIGINAL')
  )
  into v_result
  from visible;
...
comment on function public.get_application_counts_v29(timestamptz, timestamptz) is
  'RLS-compatible Application summary for a bounded reporting window. Includes tailored vs non-tailored resume counts.';
...
begin
  ...
  from (
    ...
    select p.id, p.applier_name, p.email, p.profile_status,
      ...
      count(pa.id) filter (where pa.status <> 'CANCELLED' and pa.resume_type = 'TAILORED')::integer tailored_count,
      count(pa.id) filter (where pa.status <> 'CANCELLED' and pa.resume_type = 'ORIGINAL')::integer non_tailored_count,
...
comment on function public.get_business_overview_v31(timestamptz, timestamptz) is
  'Business overview with Applier productivity, including tailored vs non-tailored Application counts.';
