-- My Applications resume filter: group tailored variants under their ORIGINAL profile.
...
comment on function public.list_my_applications_v20(text, text, integer, uuid) is
  'Applier My Applications list with status filters and ORIGINAL-profile resume grouping (includes tailored variants).';
