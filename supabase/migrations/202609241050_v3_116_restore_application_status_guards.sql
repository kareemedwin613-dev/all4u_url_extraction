-- Restore the latest status behavior after out-of-order historical migration replay.
-- This forward migration is safe on both a fresh schema and a repaired database.
-- No application rows/history are rewritten. The body combines v110 and v114.
...
begin
  ...
  if v_manager and p_priority is not null and v_priority not in ('LOW', 'NORMAL', 'HIGH', 'URGENT') then
