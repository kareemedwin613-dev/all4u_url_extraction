-- Overview Applier Performance: productivity metrics for Admin table (active days, avg/day, last activity).
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(x)order by x.applied_count desc,x.active_days desc,x.applier_name),'[]'::jsonb)into v_appliers from(
    ...
    select p.id,p.applier_name,p.email,p.profile_status,
      ...
      case when coalesce(act.active_days,0)=0 then 0 else round(count(a.id)filter(where a.applied_at>=p_from and a.applied_at<p_to)::numeric/act.active_days,1)end avg_per_day
    from actors p
...
comment on function public.get_business_overview_v31(timestamptz,timestamptz)is 'Date-windowed business overview with Applier productivity metrics for Admin reporting.';
