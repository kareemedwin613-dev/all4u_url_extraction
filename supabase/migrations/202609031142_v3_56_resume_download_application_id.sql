-- Include Application number in tailored Resume download filenames so users
-- can match downloaded files to specific job applications.

-- Drop the old 5-param overload to avoid ambiguous function resolution.
...
create or replace function public.application_resume_download_filename_v352(
  ...
  p_application_number integer default null
)
returns text
language sql
immutable
...
comment on function public.application_resume_download_filename_v352(text, text, text, text, text, integer) is
