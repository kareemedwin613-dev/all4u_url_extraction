...
const migration = await readFile(new URL("../../../supabase/migrations/202609171000_v3_100_match_model_terra.sql", import.meta.url), "utf8");
...
test("Terra model migration updates defaults and new tickets without rewriting historical scores", async t => {
  ...
  assert.deepEqual((await pg.query("select * from application_match_settings")).rows[0], { ...settings, model_id: "gpt-5.6-terra" });
  ...
  assert.equal(column.column_default, "'gpt-5.6-terra'::text");
  await assert.rejects(call(old.ticket, "claim"), /MATCH_WORKER_VERSION_MISMATCH/);

  // A different unassigned pair can now be queued under the Terra cache identity.
  ...
  assert.equal(fresh.modelId, "gpt-5.6-terra");
  assert.equal((await call(fresh.ticket, "claim")).modelId, "gpt-5.6-terra");
  const terraJob = await call(fresh.ticket, "next", { ...version, modelId: "gpt-5.6-terra" });
  assert.equal(terraJob.state, "JOB");
  assert.equal(terraJob.modelId, "gpt-5.6-terra");
