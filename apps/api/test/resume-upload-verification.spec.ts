...
test("reviewed upload stores canonical metadata as verified in one Resume insert",async()=>{
 let inserted:any;const rowId="323e4567-e89b-42d3-a456-426614174000";
 const query:any={insert:(row:any)=>{inserted=row;return query;},select:()=>query,single:async()=>({data:{id:rowId,profile_review_status:"VERIFIED",primary_category_id:metadata.primaryCategoryId,subcategory_id:null},error:null}),delete:()=>query,eq:()=>query};
 const stackQuery:any={select(){return this;},in(){return this;},order(){return this;},then(resolve:any,reject:any){return Promise.resolve({data:[{resume_id:rowId,primary_category_id:metadata.primaryCategoryId,subcategory_id:null,sort_order:0}],error:null}).then(resolve,reject);}};
 const client={storage:{from:()=>({upload:async()=>({error:null}),remove:async()=>({error:null})})},rpc:async()=>({data:[{primary_category_id:metadata.primaryCategoryId,subcategory_id:null}],error:null}),from:(table:string)=>{if(table==="resume_tech_stacks")return stackQuery;assert.equal(table,"resumes");return query;}};
