...
test("Tailoring creation derives ownership and source paths instead of trusting browser values",async()=>{let inserted:any[]=[];const resumeQuery:any={select:()=>resumeQuery,in:()=>resumeQuery,eq:async()=>({data:[{id:"223e4567-e89b-42d3-a456-426614174000",storage_path:"trusted/path.pdf"}],error:null})},tailoringQuery:any={upsert:(rows:any[])=>{inserted=rows;return tailoringQuery;},select:async()=>({data:[{resume_id:"223e4567-e89b-42d3-a456-426614174000"}],error:null})};const service=new TailoringService({forUser:(token:string)=>{assert.equal(token,"jwt");return{from:(table:string)=>table==="resumes"?resumeQuery:tailoringQuery};}}as any),result=await service.create({id:"actor-id",token:"jwt",claims:{}},{jobDescriptionId:"123e4567-e89b-42d3-a456-426614174000",matches:[{resumeId:"223e4567-e89b-42d3-a456-426614174000",matchScore:80,matchDetails:{}}]});assert.equal(inserted[0].user_id,"actor-id");assert.equal(inserted[0].source_resume_path,"trusted/path.pdf");assert.equal(result[0].status,"created");});
