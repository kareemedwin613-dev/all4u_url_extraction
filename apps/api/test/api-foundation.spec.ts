...
test("Tailoring creation, generation, and review use only protected caller-scoped RPCs",async()=>{const calls:any[]=[];const service=new TailoringService({forUser:(token:string)=>{assert.equal(token,"jwt");return{rpc:async(name:string,args:any)=>{calls.push({name,args});return{data:name==="create_tailoring_jobs_v13"?[{status:"created",resumeId:"223e4567-e89b-42d3-a456-426614174000"}]:{id:"job"},error:null};}};}}as any),user={id:"actor-id",token:"jwt",claims:{}};await service.create(user,{jobDescriptionId:"123e4567-e89b-42d3-a456-426614174000",matches:[{resumeId:"223e4567-e89b-42d3-a456-426614174000",matchScore:80,matchDetails:{}}]});await service.requestApplication(user,"323e4567-e89b-42d3-a456-426614174000");await service.input(user,"423e4567-e89b-42d3-a456-426614174000");await service.preview(user,"423e4567-e89b-42d3-a456-426614174000",{generatedAt:"2026-08-03T12:00:00.000Z",result:{summary:"x"}});await service.review(user,"423e4567-e89b-42d3-a456-426614174000",{action:"APPROVE",preview:{summary:"x"},notes:"reviewed",expectedUpdatedAt:"2026-08-03T12:00:00.000Z"});await service.reviews(user,"423e4567-e89b-42d3-a456-426614174000");await service.cancel(user,"423e4567-e89b-42d3-a456-426614174000");assert.deepEqual(calls.map(x=>x.name),["create_tailoring_jobs_v13","request_application_tailoring_v13","get_tailoring_job_input_v13","submit_tailoring_preview_v13","review_tailoring_preview_v14","get_tailoring_job_reviews_v14","cancel_tailoring_job_v13"]);assert.equal(calls[0].args.p_matches[0].sourceResumePath,undefined);assert.deepEqual(calls[3].args.p_preview,{summary:"x"});assert.equal(calls[4].args.p_expected_updated_at,"2026-08-03T12:00:00.000Z");});
