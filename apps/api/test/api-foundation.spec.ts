...
test("Application service forwards only allowlisted RPC arguments with the caller JWT",async()=>{
  ...
  assert.equal(call.name,"list_applications_v360");assert.equal(call.args.p_search,"Acme");assert.equal(call.args.p_work_status,"IN_PROGRESS");assert.equal(call.args.p_limit,25);assert.equal(call.args.p_offset,0);assert.equal("unexpected" in call.args,false);
});
