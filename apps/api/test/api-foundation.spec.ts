...
test("v1.6 materialization renders, uploads, hashes, and finalizes through one user-scoped client",async()=>{const calls:any[]=[],uploads:any[]=[],removals:any[]=[];const started={alreadyMaterialized:false,jobId:"423e4567-e89b-42d3-a456-426614174000",applicationId:"323e4567-e89b-42d3-a456-426614174000",applicationNumber:19,materializationToken:"523e4567-e89b-42d3-a456-426614174000",targetBucket:"tailored-resumes",targetPath:"owner/job/resume.docx",filename:"resume.docx",candidate:{name:"Alex Example",email:"alex@example.com"},sourceStructuredContent:{professional_experience:[{id:"exp-1",company:"Source Co",job_title:"Engineer",experience_details:"source",is_current:true}],education:[],certifications:[]},approvedPreview:{summary:"A sufficiently complete approved summary for a tailored resume artifact.",professionalExperience:[{sourceExperienceId:"exp-1",tailoredDetails:"Delivered supported and measurable engineering improvements for customers."}],skills:["SQL"]}};const service=new TailoringService({forUser:(token:string)=>{assert.equal(token,"jwt");return{rpc:async(name:string,args:any)=>{calls.push({name,args});return{data:name==="begin_tailoring_materialization_v16"?started:{jobId:started.jobId,status:"COMPLETED",tailoredResumeId:"623e4567-e89b-42d3-a456-426614174000",tailoredResumeNumber:42},error:null};},storage:{from:(bucket:string)=>({remove:async(paths:string[])=>{removals.push({bucket,paths});return{data:[],error:null};},upload:async(path:string,bytes:Buffer,options:any)=>{uploads.push({bucket,path,bytes,options});return{data:{path},error:null};}})}};}}as any),user={id:"actor-id",token:"jwt",claims:{}};const result:any=await service.materialize(user,started.jobId);assert.equal(result.tailoredResumeNumber,42);assert.deepEqual(calls.map(x=>x.name),["begin_tailoring_materialization_v16","finalize_tailoring_materialization_v16"]);assert.equal(uploads.length,1);assert.equal(uploads[0].bytes.subarray(0,2).toString(),"PK");assert.equal(uploads[0].options.upsert,false);assert.match(calls[1].args.p_file_sha256,/^[0-9a-f]{64}$/);assert.equal(calls[1].args.p_materialization_token,started.materializationToken);assert.deepEqual(removals,[{bucket:"tailored-resumes",paths:[started.targetPath]}]);});

test("v1.6 upload failure returns the approved job to a retryable state",async()=>{const calls:string[]=[],started={alreadyMaterialized:false,jobId:"423e4567-e89b-42d3-a456-426614174000",applicationNumber:19,materializationToken:"523e4567-e89b-42d3-a456-426614174000",targetBucket:"tailored-resumes",targetPath:"owner/job/resume.docx",filename:"resume.docx",candidate:{name:"Alex"},sourceStructuredContent:{professional_experience:[{id:"exp-1",company:"Co",job_title:"Engineer",experience_details:"source"}],education:[],certifications:[]},approvedPreview:{summary:"A complete approved summary for the candidate and target role.",professionalExperience:[{sourceExperienceId:"exp-1",tailoredDetails:"A supported professional experience description for this role."}],skills:["SQL"]}};const service=new TailoringService({forUser:()=>({rpc:async(name:string)=>{calls.push(name);return{data:name==="begin_tailoring_materialization_v16"?started:{status:"APPROVED"},error:null};},storage:{from:()=>({remove:async()=>({data:[],error:null}),upload:async()=>({data:null,error:{message:"storage unavailable"}})})}})}as any);await assert.rejects(()=>service.materialize({id:"actor",token:"jwt",claims:{}},started.jobId),error=>error instanceof ApiException&&error.getStatus()===502);assert.deepEqual(calls,["begin_tailoring_materialization_v16","fail_tailoring_materialization_v16"]);});
