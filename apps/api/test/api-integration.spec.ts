...
test("simple JD decisions are restricted, validated, and returned",async()=>{
  ...
  await request(app.getHttpServer()).post("/api/v1/job-descriptions/bulk-delete").set("Authorization","Bearer token").send({jobDescriptionIds:[id]}).expect(201).expect(({body})=>{assert.equal(body.data.succeeded,1);assert.equal(body.data.results[0].ok,true);});
  roles=["APPLIER"];
  ...
  await request(app.getHttpServer()).post("/api/v1/job-descriptions/bulk-delete").set("Authorization","Bearer token").send({jobDescriptionIds:[id]}).expect(403);
});
...
test("Profile, Admin, overview, and tailoring routes enforce the final backend boundary",async()=>{
  ...
  await request(app.getHttpServer()).patch(`/api/v1/tailoring-jobs/${id}/format`).set("Authorization","Bearer token").send({renderFormat:"DOCX",expectedUpdatedAt:"2026-08-03T12:00:00.000Z"}).expect(400);
