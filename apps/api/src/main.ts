...
export async function createApiApplication(options: { serverless?: boolean } = {}): Promise<INestApplication> {
  const env = environment(), app = await NestFactory.create(AppModule, { bufferLogs: true }), logger = app.get(JsonLogger);
  ...
  app.enableCors({ origin: origins, credentials: false, methods: ["GET", "POST", "PATCH", "DELETE", "OPTIONS"], allowedHeaders: ["Authorization", "Content-Type", "Idempotency-Key", "X-Request-ID"], exposedHeaders: ["X-Request-ID"] });
