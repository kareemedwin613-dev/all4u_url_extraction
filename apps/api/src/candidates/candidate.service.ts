...
export class CandidateService {
  ...
  structured(user: AuthenticatedUser, id: string, value: UpdateResumeStructuredContentDto) { return this.rpc(user, "update_resume_structured_content_v384", { p_resume_id: id, p_summary: value.summary?.trim() || "", p_skills: value.skills?.trim() || "", p_employment: value.employment, p_education: value.education, p_certifications: value.certifications, p_education_legacy_text: value.educationLegacyText == null ? "" : String(value.educationLegacyText) }, "The structured Resume could not be updated."); }
