...
export async function renderTailoredResumePdf(input:JsonRecord):Promise<Buffer>{
  ...
  // Positioning label under the name; employment titles below are never changed.
  const headline=resolveResumeHeadline(input);if(headline)document.font(fonts.bold).fontSize(bodySize+2).fillColor(accent).text(headline,{align});
  ...
  // Skills sit directly under the summary so the matching stack is visible in the first scan.
  section("Skills");
  const skillGroups=renderedSkillGroups(preview.skills,preview.skillGroups),labelWidth=spec.compact?110:125,rowGap=spec.compact?3:5,contentWidth=document.page.width-margin*2;
