...
import {SaveResumeAnswerDto,SaveResumeAnswersBulkDto} from "./resume-answer.dto.js";
...
export class ResumeAnswerController{
  ...
  @Patch() @ApiOperation({summary:"Save the manually configured Resume Answer Library in one request"})
  async saveAll(@Req()request:ApiRequest,@Param("resumeId",new ParseUUIDPipe({version:"4"}))resumeId:string,@Body(new DtoValidationPipe(SaveResumeAnswersBulkDto))body:SaveResumeAnswersBulkDto){return this.response(request,await this.service.saveAll(request.user!,resumeId,body.answers));}
