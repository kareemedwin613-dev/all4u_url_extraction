...
export class ResumeService{
  ...
  async upload(user:AuthenticatedUser,metadata:any,file:any){
    ...
    const id=randomUUID(),path=`${user.id}/${id}/${safeName(file.originalname)}`,client=this.supabase.forUser(user.token),row={
      ...
      resume_type:"ORIGINAL",parent_resume_id:null,resume_name:clean(metadata.resumeName),primary_category_id:metadata.primaryCategoryId,subcategory_id:metadata.subcategoryId||null,seniority:metadata.seniority||"UNSPECIFIED",skills:array(metadata.skills),industries:array(metadata.industries),
