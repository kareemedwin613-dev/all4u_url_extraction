...
export class JobDescriptionController {
  ...
  async create(@Req() request: ApiRequest, @Res({ passthrough: true }) response: Response, @Headers("idempotency-key") idempotencyKey: string | undefined, @Body(JdValidationPipe) body: CreateJobDescriptionDto) {
    ...
    return { data: { id: row.id, company: row.company, jobTitle: row.job_title, sourceUrl: row.source_url, createdAt: row.created_at, reviewStatus:row.review_status, duplicate: result.duplicate, duplicateReason: result.duplicateReason, categoryId: row.category_id, subcategoryId: row.subcategory_id, industryDomainCategoryId: row.industry_domain_category_id, seniority: row.seniority, locationText: row.location_text, workArrangement: row.work_arrangement, clearanceRequirements: row.clearance_requirements, travelRequired: row.travel_required, travelDetails: row.travel_details, salaryMin: row.salary_min, salaryMax: row.salary_max, salaryCurrency: row.salary_currency, salaryPeriod: row.salary_period, salaryText: row.salary_text, sourceWebsite: row.source_site, descriptionText: row.description_text, detectedSkills: row.detected_skills, captureMethod: row.capture_method, extractionConfidence: row.extraction_confidence }, requestId: request.requestId };
  }
}
