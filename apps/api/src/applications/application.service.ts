...
import { renderCoverLetterPdf } from "../resumes/cover-letter-pdf.renderer.js";
...
const screenshotMime=(file:any)=>{const allowed=SCREENSHOT_MIME_TYPES;if(file?.mimetype&&allowed.has(file.mimetype))return file.mimetype;const ext=String(file?.originalname||"").split(".").pop()?.toLowerCase();return SCREENSHOT_MIME_BY_EXT[ext||""]||"";};
...
@Injectable()export class ApplicationService{constructor(@Inject(SupabaseService)private readonly supabase:SupabaseService){}
  ...
  update=async(u:AuthenticatedUser,id:string,m:any)=>{const row:any=await this.rpc(u,"update_application_status_v101",{p_application_id:id,p_status:m.status,p_application_url:m.applicationUrl||null,p_applied_at:m.appliedAt||null,p_notes:m.notes==null?null:String(m.notes),p_priority:m.priority??null,p_due_at:m.dueAt??null},"The Application could not be updated.");const normalized=this.normalized(row);const siblingsBlocked=Number(row?.siblings_blocked??row?.siblingsBlocked??row?.siblings_cancelled??row?.siblingsCancelled)||0;return{...normalized,jobApplicationBlocked:Boolean(row?.job_application_blocked??row?.jobApplicationBlocked),siblingsBlocked,siblingsCancelled:siblingsBlocked};};
  ...
  async resumeUrl(u:AuthenticatedUser,id:string){const file:any=await this.rpc(u,"get_application_resume_download_v17",{p_application_id:id},"The Resume is not available for this Application."),number=Number(file?.resumeNumber),type=String(file?.resumeType||""),mime=String(file?.mimeType||""),size=Number(file?.fileSizeBytes);if(!file?.bucket||!file?.path||!file?.filename||!Number.isSafeInteger(number)||number<1||!["ORIGINAL","TAILORED"].includes(type)||!["application/pdf","application/vnd.openxmlformats-officedocument.wordprocessingml.document","text/plain"].includes(mime)||!Number.isSafeInteger(size)||size<1||size>5242880)throw new ApiException("APPLICATION_RESUME_METADATA_INVALID","The attached Resume metadata is invalid.",HttpStatus.CONFLICT);const{data,error}=await this.supabase.forUser(u.token).storage.from(file.bucket).createSignedUrl(file.path,90);if(error||!data?.signedUrl)failure(error,"The private Resume file could not be opened.");return{signedUrl:data.signedUrl,expiresInSeconds:90,filename:file.filename,mimeType:mime,fileSizeBytes:size,resumeNumber:number,resumeType:type,candidateName:file.candidateName||null,resumeName:file.resumeName||null,applicationNumber:Number(file?.applicationNumber)||null};}
  // Tailored letter when the attached Resume has one, else the original's base letter; rendered like the dashboard download.
  async coverLetter(u:AuthenticatedUser,id:string){const letter:any=await this.rpc(u,"get_application_cover_letter_v123",{p_application_id:id},"The cover letter is not available for this Application.");const number=Number(letter?.applicationNumber),name=String(letter?.candidateName||"Candidate").trim()||"Candidate";if(!String(letter?.text||"").trim()||!["TAILORED","BASE"].includes(String(letter?.kind)))throw new ApiException("APPLICATION_COVER_LETTER_METADATA_INVALID","The cover letter metadata is invalid.",HttpStatus.BAD_GATEWAY);const bytes=await renderCoverLetterPdf({candidate_name:name,candidate_email:letter.candidateEmail,candidate_phone:letter.candidatePhone,address_city:letter.city,address_state_region:letter.stateRegion,linkedin_url:letter.linkedinUrl,cover_letter_text:letter.text});return{kind:letter.kind,filename:safeName(`${name} Cover Letter${Number.isSafeInteger(number)&&number>0?` - App ${number}`:""}.pdf`).replace(/_/g," "),mimeType:"application/pdf",contentBase64:bytes.toString("base64"),resumeNumber:Number(letter.resumeNumber)||null,applicationNumber:Number.isSafeInteger(number)?number:null};}
  ...
  async addScreenshot(u:AuthenticatedUser,id:string,file:any){const mime=screenshotMime(file);if(!file||file.size<1||file.size>5242880||!mime||!SCREENSHOT_MIME_TYPES.has(mime))throw new ApiException("APPLICATION_SCREENSHOT_INVALID","Use a PNG, JPG, WEBP, or PDF file between 1 byte and 5 MiB.",HttpStatus.BAD_REQUEST);const path=`${id}/${randomUUID()}-${safeName(file.originalname)}`,client=this.supabase.forUser(u.token),uploaded=await Promise.race([client.storage.from("application-screenshots").upload(path,file.buffer,{contentType:mime,upsert:false}),new Promise<never>((_,reject)=>setTimeout(()=>reject(new ApiException("UPSTREAM_TIMEOUT","The screenshot upload timed out. Try again.",HttpStatus.GATEWAY_TIMEOUT)),55000))]);if(uploaded.error)failure(uploaded.error,"The screenshot file could not be uploaded.");try{return await this.rpc(u,"attach_application_screenshot",{p_application_id:id,p_storage_path:path,p_original_filename:file.originalname,p_mime_type:mime,p_file_size_bytes:file.size},"The screenshot could not be attached.");}catch(error){await client.storage.from("application-screenshots").remove([path]);throw error;}}
  async removeScreenshot(u:AuthenticatedUser,id:string,screenshotId:string){const client=this.supabase.forUser(u.token),{data:existing,error:lookupError}=await client.from("application_screenshots").select("id,storage_bucket,storage_path").eq("application_id",id).eq("id",screenshotId).maybeSingle();if(lookupError)failure(lookupError,"The screenshot could not be loaded.");if(!existing)throw new ApiException("APPLICATION_SCREENSHOT_NOT_FOUND","The screenshot was not found.",HttpStatus.NOT_FOUND);const result:any=await this.rpc(u,"remove_application_screenshot",{p_screenshot_id:screenshotId},"The screenshot could not be removed.");const storagePath=String(result?.storagePath||existing.storage_path||""),storageBucket=String(result?.storageBucket||existing.storage_bucket||"application-screenshots");if(storagePath){try{await Promise.race([client.storage.from(storageBucket).remove([storagePath]),new Promise<never>((_,reject)=>setTimeout(()=>reject(new Error("STORAGE_REMOVE_TIMEOUT")),10000))]);}catch{}}return result;}
