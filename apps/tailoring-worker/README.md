...
Generated previews are validated before they can be saved or submitted. The worker protects the output shape, preserves the exact source-experience ID/count/order mapping used to render employer, title, and date fields, and rejects refusals. It also enforces the fixed-contract rules the schema cannot express: a single-paragraph summary of at most 160 words, and bullets-only `tailoredDetails` (other bullet markers are normalized to `- `) with no more bullets per role than `ROLE_TARGETS_JSON` allows. A rejected output gets one repair attempt, which appends the rejection reason to the saved prompt; a second rejection fails the item as `VALIDATION_FAILED`. Facts, metrics, keyword coverage, action verbs, and wording are not gated.

The preview retains one prioritized flat `skills` list capped at 80 items for ATS coverage and adds `skillGroups` for readable category-aligned PDF rendering. JD skills come first, but only those the candidate supports: in the Resume skills list, or named in the Resume summary or role details (terms shorter than three characters must be in the skills list). The candidate's own skills follow, then model additions that also appear in the Resume text. A JD skill the Resume does not support is never added. Missing or legacy group metadata is reconciled automatically without changing the database schema.
...
Tailoring defaults to `TAILORING_CODEX_MODEL=gpt-5.6-sol`, `TAILORING_CODEX_REASONING_EFFORT=medium`, disabled reasoning summaries, and `TAILORING_CODEX_SERVICE_TIER=default` (standard processing). Matching and JD review also use standard processing by default. Override the model with `gpt-5.6-luna` or `gpt-5.6-terra`, or set reasoning to `none`, `low`, `high`, or `xhigh`, when a different latency/quality tradeoff is needed. Fast processing is an explicit opt-in through `TAILORING_CODEX_SERVICE_TIER=fast` and may consume more credits. To ensure standard processing on another worker machine, replace any existing `fast` or `auto` override with `default` in its environment or `apps/tailoring-worker/.env`, and restart the worker. Model, reasoning, concurrency, and prompt settings do not change when switching the service tier.
...
If the terminal closes, create a replacement command from the same batch. The server revokes the old capability and safely requeues its leased item. Validation failures are not retried beyond the worker's single repair attempt; other transient worker/API failures can be selected for retry from the batch page. When an item failed while rendering, uploading, or finalizing its PDF, a retry keeps the approved preview and the worker resubmits it without another model call (migration v3.118).
