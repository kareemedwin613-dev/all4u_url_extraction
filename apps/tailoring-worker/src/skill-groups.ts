import type { TailoredSkillGroup } from "./types.js";
...
const groupNames=new Set<string>(SKILL_GROUP_NAMES);
...
export function reconcileSkillGroups(skills:string[],proposed:TailoredSkillGroup[]=[]):TailoredSkillGroup[]{
  const prioritized:string[]=[];const unique=new Set<string>();
  for(const raw of skills){const skill=clean(raw),skillKey=key(skill);if(!skill||unique.has(skillKey))continue;unique.add(skillKey);prioritized.push(skill);if(prioritized.length===MAX_TAILORED_SKILLS)break;}
  const canonical=new Map(prioritized.map(skill=>[key(skill),skill])),assigned=new Set<string>(),buckets=new Map<SkillGroupName,string[]>();
  const add=(name:SkillGroupName,skill:string)=>{const skillKey=key(skill),value=canonical.get(skillKey);if(!value||assigned.has(skillKey))return;assigned.add(skillKey);const bucket=buckets.get(name)||[];bucket.push(value);buckets.set(name,bucket);};
  for(const group of proposed){const name=groupNames.has(group.name)?group.name as SkillGroupName:"Additional Skills";for(const skill of group.skills||[])add(name,skill);}
  for(const skill of prioritized)if(!assigned.has(key(skill)))add(inferSkillGroup(skill),skill);
  return SKILL_GROUP_NAMES.flatMap(name=>{const grouped=buckets.get(name)||[];return grouped.length?[{name,skills:grouped}]:[];});
}
